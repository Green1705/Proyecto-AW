...
router
  ...
  .post((req, res) => {
    const query =
      "SELECT id_usuario, nombre, apellido_paterno, apellido_materno, rol, email, password, id_concesionario, contraste, tamanio_texto FROM usuario WHERE email = ? AND password = ?";
    pool.query(query, [req.body.email, req.body.password], (err, results) => {
      if (err) {
        ...
        if (results.length > 0) {
          ...
          req.session.contrastPreference = user.contraste || "normal";
          req.session.textSizePreference = user.tamanio_texto || "normal";
